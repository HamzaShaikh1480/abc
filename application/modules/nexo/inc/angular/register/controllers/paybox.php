	...
	$scope.confirmOrder			=	function( action ) {
		if( action ) {
			if( $scope.cart.paidSoFar > 0 ) {
				...
				if( $scope.cart.paidSoFar < NexoAPI.ParseFloat( v2Checkout.CartToPay ) ) {
					NexoAPI.Notify().warning( '<?php echo _s('Une erreur s\'est produite', 'nexo');?>', '<?php echo _s( 'Seul les commandes complètes sont autorisées.', 'nexo' );?>' );
					return false;
				}
